...
class ProductSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        instance.name = validated_data.get('name', instance.name) # Aktualisiert den Namen, falls im validated_data vorhanden, ansonsten behält er den alten Namen bei.
        ...
        # Lagerbestand aktualisieren, falls vorhanden
        if stock_data:
            stock = instance.stock
            stock.quantity = stock_data.get('quantity', stock.quantity)
            stock.save()

        return instance  # Gibt die aktualisierte Produktinstanz zurück.
